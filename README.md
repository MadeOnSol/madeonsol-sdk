...
- **Replayed frames.** `evt.mode` is `"ring"` (the server's in-memory buffer, same `seq`) or `"durable"` (rebuilt from storage after a restart or an old cursor: `seq` is `null`, and `partial: true` + `missing: [...]` name live fields the row cannot reproduce — absent, never guessed). A `seq: null` frame never moves the cursor's `seq` back; it keeps the last real one and advances `ts`. Frames the server re-sends after its own event-bus reconnect carry `replayed: true, recovered: "bus"` and arrive live. If the server refuses a resume (`subscribed.resume.accepted === false`, `warning` code `replay_in_progress`) no replay follows and the client does not wait for one.
