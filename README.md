...
- **Committed cursor vs progress.** `getCursor()` is the COMMITTED, safe cursor — persist and resume from this one. `getProgress()` is what has been received and handled (replayed frames included) and is not safe to resume from. Live frames commit as they are handled. Replayed frames never commit: the server replays channel by channel, so only a `replay_end` with `complete: true` (and no channel flagged incomplete or `best_effort`) commits, at the server's `last_seq` / `last_ts`. If a recovery is incomplete, or the socket closes mid-replay, the committed cursor stays at the pre-resume point, and live frames after it are delivered but not committed until a later recovery completes (`isRecoveryIncomplete()`). **Trade-off:** the next reconnect re-requests the unrecovered range from the old cursor, and what arrives twice is dropped by id. Call `acceptGap()` once you have backfilled the range the `gap` event named (it carries the server's `nextStep` when there is one), or decided to skip it.
...
- **Replayed frames.** `evt.mode` is `"ring"` (the server's in-memory buffer, same `seq`) or `"durable"` (rebuilt from storage after a restart or an old cursor: `seq` is `null`, and `partial: true` + `missing: [...]` name live fields the row cannot reproduce — absent, never guessed). Frames the server re-sends after its own event-bus reconnect carry `replayed: true, recovered: "bus"` and arrive live. If the server refuses a resume (`subscribed.resume.accepted === false`, `warning` code `replay_in_progress`) no replay follows and the client does not wait for one.
